#!/usr/bin/env python3
"""Analyze all unique pairwise pre_xyz distances for five high-output FAFB neurons.
...
def extract_selected_coordinates(
    ...
    qc = {
        ...
        "rows_with_any_missing_or_nonnumeric_pre_coordinate": 0,
    }
    ...
    for chunk in pd.read_csv(
        ...
        qc["rows_with_any_missing_or_nonnumeric_pre_coordinate"] += int(
            (~coord_valid).sum()
        )
...
def duplicate_coordinate_stats(coords: np.ndarray) -> dict[str, int]:
    ...
    return {
        "unique_pre_xyz_count": int(len(multiplicities)),
...
def render_histogram_figure(
    ...
    draw.text(
        (60, 30),
        "FAFB v783 Princeton table: all unique pairwise distances between outgoing pre_xyz rows",
        font=title_font,
        fill="#18212C",
    )
    draw.text(
...
def main() -> None:
    ...
    figure_png = output_dir / "prexyz_pairwise_distance_histograms.png"
    figure_pdf = output_dir / "prexyz_pairwise_distance_histograms.pdf"
    ...
    notes_path.write_text(
        f"""# Princeton pre_xyz pairwise-distance analysis
...
- Presynaptic coordinate columns: `{', '.join(COORD_COLS)}`
...
- Rows with any missing/nonnumeric/nonfinite `pre_xyz` coordinate: {coord_qc['rows_with_any_missing_or_nonnumeric_pre_coordinate']:,}
- Selected rows dropped for invalid coordinates: {sum(selected_invalid_coords.values()):,}
- Exact duplicate coordinates were retained, because repeated `pre_xyz` rows can
